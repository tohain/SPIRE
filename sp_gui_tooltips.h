...
class tooltips {
 public:
  ...
  const std::string nr_points_xy_tooltip = "The number of samples/points to put in the projection box in x and y direction. Corresponds to the final resolution of the projection. IMPORTANT: large (>500) values significantly increase memory and ressource usage and may crash your maschine";
  const std::string nr_points_z_tooltip = "Number of samples/points to put in the projection box in z direction. Increasing this value may help if artifacts and \"steps\" in the image occur. IMPORTANT: large (>250) values significantly increase memory and ressource usage and may crash your maschine";
   
  const std::string invert_tooltip = "Inverts the colors in the picture. If inverted the pixel with the highest value is black.";
  const std::string autoupdate_tooltip = "Automatically update projection as soon as a parameter changes, can result in significant delays and lags in the usage of the user interface!";
  ...
  const std::string button_measure = "Start the measurements for the current settings. This may take up to several minutes. Wait for the status indicator to turn green.";
